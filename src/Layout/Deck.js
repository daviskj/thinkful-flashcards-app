...
const Deck = () => {
  ...
  // define state of deck/setDeck to cards with empty array, map through cards
  const { deckId } = useParams();
  const history = useHistory();

  useEffect(() => {
    readDeck(deckId).then(setDeck); 
    //get deckId, change with setDeck, run when every deckId changes
  }, [deckId]);
  ...
  function handleDeckDelete(deckId) { // delete specific deck
    ...
    if (confirmDelete) { 
      // call deleteDeck to access deckId of specific deck you want to delete 
      deleteDeck(deckId);
      history.push("/");
    }
  }
  function handleCardDelete(cardId) { 
    ...
    if (confirmDelete) { 
      // call deleteCard to access cardId of specific card you want to delete 
      deleteCard(cardId);
      readDeck(deckId).then(setDeck);
    }
  }
